...
{
	/*
	int length = getFrameDataLength();
	uint8_t *temp = getFrameData();
...
bool XBee::begin(int comNum, int baudrate)
{
	char mode[] = { '8', 'N', '1', 0 };
	comPortNumber = comNum;
	if (RS232_OpenComport(comNum, baudrate, mode))
	{
...
void XBee::end(int comNum)
{
	RS232_CloseComport(comNum);	
}
...
bool XBee::available() 
{
	int availableDataNum = RS232_PollComport(comPortNumber, &availableByte, 1);
	if (availableDataNum > 0)
	{
		isAvailable = 1;
		//printf("%c ",availableDataNum);
	}
...
{
	RS232_SendByte(comPortNumber, (unsigned char)val);
}
		...
		default:
			...
			if (_pos == (_response.getPacketLength() + 3)) {
				...
				if ((_checksumTotal & 0xff) == 0xff) {
					...
					_response.setErrorCode(XBEE_NO_ERROR);
				}
				else {
					// checksum failed
...
int XBeeAddress64::isEqual(XBeeAddress64 addr1)
{
	int equal = 0;
	if (addr1.getLsb() == _lsb && addr1.getMsb() == _msb)
	{
		equal = 1;
	}
	else
